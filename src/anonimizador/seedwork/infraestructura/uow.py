...
class UnidadTrabajo(ABC):
    ...
    def _publicar_eventos_dominio(self, batch):
        for evento in self._obtener_eventos(batches=[batch]):
            print('******Evento dominio publicar desde Anonimizador**********')
            print(evento)
    ...
    def _publicar_eventos_post_commit(self):
        try:
            for evento in self._obtener_eventos():
                print('***Publicar eventos post commit desde Anonimizador*********************')
                print(evento)
                dispatcher.send(signal=f'{type(evento).__name__}Integracion', evento=evento)
        except:
            logging.error('ERROR: Suscribiendose al tópico de eventos desde anonimizador!')
            traceback.print_exc()
